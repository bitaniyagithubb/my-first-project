...
from . models import Dam,Department,Staff, Fish, TimeLine,Debtors #Sales,
...
class OrdersAll(LoginRequiredMixin,ListView):
    login_url = 'fms:login'
    model = Order
    template_name = 'fmsapp/orders/index.html'
...
class Login(LoginView):
    ...
    def get_success_url(self):
        ...
        if user.is_superuser:
            url = resolve_url('fmsapp:dashboard')  # Redirect superuser to 'fmsapp:dashboard'
        else:
            url = resolve_url('sales:sales')  # Redirect regular users to 'sales:sales'
